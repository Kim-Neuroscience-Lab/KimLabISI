...
class HardwareComponent:
    ...
    def get_status_history(self, hours: int = 24) -> List[tuple[datetime, HardwareStatus]]:
        ...
        cutoff_time = datetime.now() - timedelta(hours=hours)
        return [(timestamp, status) for timestamp, status in self._status_history
                if timestamp >= cutoff_time]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "last_calibration": self._metrics.last_calibration.isoformat() if self._metrics.last_calibration else None,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> HardwareComponent:
        ...
        component._metrics = HardwareMetrics(
            ...
            last_calibration=datetime.fromisoformat(metrics_data["last_calibration"]) if metrics_data.get("last_calibration") else None,
...
class HardwareSystem:
    ...
    def get_components_by_type(self, hardware_type: HardwareType) -> List[HardwareComponent]:
        ...
        return [comp for comp in self._components.values()
                if comp.hardware_type == hardware_type]
        ...
        for comp_id, comp_data in data["components"].items():
            ...
            if hardware_type == HardwareType.CAMERA:
                component = Camera(
                    ...
                    comp_data["name"],
                    CameraModel(comp_data["model"]) if comp_data["model"] in [m.value for m in CameraModel] else CameraModel.GENERIC,
                ...
                component = Display(
                    comp_data["hardware_id"],
                    comp_data["name"],
                    DisplayType(comp_data["model"]) if comp_data["model"] in [d.value for d in DisplayType] else DisplayType.LCD_MONITOR,
                    comp_data.get("serial_number")
                )
            else:
