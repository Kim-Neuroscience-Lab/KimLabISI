...
class StartupCoordinator:
                ...
                if not current_params.monitor.selected_display:
                    ...
                    secondary_display = next(
                        (
                            ...
                            if not d.is_primary and d.is_available
                        ),
                        None,
                    )
                    target_display = secondary_display or next(
                        (d for d in available_displays if d.is_available), None
                    )
                    ...
                    if target_display:
                        ...
                        display_updates.update(
                            {
                                "monitor_fps": (
                                    int(target_display.refresh_rate)
                                    if target_display.refresh_rate > 0
                                    else -1
                                ),
    ...
    async def _wait_for_frontend_ready(
        self, backend_instance, timeout: float = 10.0
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                "Sent startup coordination command, waiting for frontend response..."
            )
