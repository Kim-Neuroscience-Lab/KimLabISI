...
class FourierAnalyzer:
    ...
    async def analyze_isi_signal(
        ...
        try:
            ...
            self._validate_inputs(
                frames, stimulus_angles, stimulus_frequency_hz, sampling_frequency_hz
            )
            ...
            # Apply preprocessing
            frames_proc = await self._preprocess_frames(frames_proc, apply_windowing, window_type)
            ...
            coherence_map = self._calculate_coherence(
                fourier_results["complex_response"], fourier_results["total_power"]
            )
            ...
            logger.info(
                "Fourier analysis completed. Quality metrics: "
                + "mean_coherence= %s, " % analysis_metadata["mean_coherence"]
                + "snr= %s" % analysis_metadata["signal_to_noise_ratio"]
                + "snr= %s" % analysis_metadata["signal_to_noise_ratio"]
            )
    ...
    async def _preprocess_frames(
        self, frames: Union[np.ndarray, "cp.ndarray"], apply_windowing: bool, window_type: str
    ...
    async def _kalatsky_fourier_analysis(
        ...
        logger.debug(
            f"Stimulus frequency {stimulus_frequency_hz} Hz maps to bin {stimulus_bin} "
