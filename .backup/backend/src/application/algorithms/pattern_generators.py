...
from .spherical_transform import create_spherical_transform_from_spatial_config
...
class PatternGenerator:
    ...
    def _setup_coordinate_grids(self):
        ...
        self.R_degrees = np.linalg.norm(
            np.stack([self.X_degrees, self.Y_degrees], axis=-1), axis=-1
        )
        self.Theta_radians = np.arctan2(self.Y_degrees, self.X_degrees)
        self.Theta_degrees = np.degrees(self.Theta_radians)

    def generate_frame(
        self, stimulus_params: StimulusGenerationParams, frame_index: int
    ) -> np.ndarray:
        """
    ...
    def _generate_horizontal_bar(
        self, params: StimulusGenerationParams, frame_index: int
    ) -> np.ndarray:
    ...
    def _add_fixation_cross(
        self, frame: np.ndarray, params: StimulusGenerationParams
    ) -> np.ndarray:
        """Add fixation cross to existing frame"""
        cross_size_pixels = params.fixation_size_degrees * self.pixels_per_degree / 2

        # Create cross masks
        horizontal_mask = (np.abs(self.Y_degrees) <= 0.1) & (
            np.abs(self.X_degrees) <= params.fixation_size_degrees / 2
        )
        vertical_mask = (np.abs(self.X_degrees) <= 0.1) & (
            np.abs(self.Y_degrees) <= params.fixation_size_degrees / 2
        )
        ...
        fixation_luminance = (
            0.299 * params.fixation_color[0]
            + 0.587 * params.fixation_color[1]
            + 0.114 * params.fixation_color[2]
        )
...
def create_horizontal_bar_params_from_parameter_system(
    parameter_set_id: str = "marshel_2011_defaults",
...
def create_vertical_bar_params_from_parameter_system(
    parameter_set_id: str = "marshel_2011_defaults",
) -> StimulusGenerationParams:
...
def create_polar_wedge_params_from_parameter_system(
    parameter_set_id: str = "marshel_2011_defaults",
) -> StimulusGenerationParams:
