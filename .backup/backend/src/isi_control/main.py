...
class ISIMacroscopeBackend:
    ...
    def __init__(self, development_mode: bool = False, host: str = "localhost", port: int = 8765):
        ...
        self.stimulus_dataset_repository = None
        self.acquisition_session_repository = None
        self.analysis_result_repository = None
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.stimulus_dataset_repository = StimulusDatasetRepository(
                base_path=datasets_directory,
                error_handler=self.error_handler
            )
            self.acquisition_session_repository = AcquisitionSessionRepository(
                base_path=datasets_directory,
                error_handler=self.error_handler
            )
            ...
            # IPC Server and Message Dispatcher
            self.ipc_server = IPCServer()
            self.message_dispatcher = MessageDispatcher()

            logger.info(">>> Infrastructure layer initialized")

            # 3. Initialize Application Layer - Services and Handlers
            logger.info(">>> Initializing application services and handlers...")
            ...
            self.state_persistence = StatePersistenceService(
                persistence_path=storage_directory / "state",
                hardware_system=self.hardware_system
            )

            # Query Handler (needs to be initialized before communication service)
            self.query_handler = QueryHandler(
                workflow_orchestrator=self.workflow_orchestrator,
                hardware_system=self.hardware_system,
                stimulus_repo=self.stimulus_dataset_repository,
                session_repo=self.acquisition_session_repository,
                analysis_repo=self.analysis_result_repository
            )

            # State Broadcaster (needs IPC server)
            self.state_broadcaster = StateBroadcaster(
                ipc_server=self.ipc_server
            )

            # Command Handler (self-initializing)
            self.command_handler = CommandHandler()

            # Communication Service (needs all handlers and dispatcher)
            ...
            # Monitoring Service (needs hardware system, workflow orchestrator, and state broadcaster)
            ...
            logger.info(">>> Application services and handlers initialized")
            ...
            self.hardware_calibration_use_case = HardwareCalibrationUseCase(
                ...
                camera_calibrator=CameraCalibrator(
                    calibration_data_path=calibration_directory / "camera"
                ),
                display_calibrator=DisplayCalibrator(
                    calibration_data_path=calibration_directory / "display",
                    colorimeter_available=False,  # Development mode
                    photometer_available=False   # Development mode
                ),
                error_handler=self.error_handler
            )
            ...
            # TODO: Fix DataAcquisitionUseCase and SessionManagementUseCase constructors
            # They need ExperimentRepositoryInterface and StimulusGenerationUseCase implementations
            # For now, setting to None to test other constructor fixes
            self.data_acquisition_use_case = None
            self.session_management_use_case = None
            ...
            # NOTE: Message handlers are automatically registered in CommunicationService._setup_message_handlers()
            # which is called during the service start process.

            # Register use cases with command handler
            # TODO: Re-enable when use cases are properly initialized
    ...
    async def _perform_health_check(self):
        ...
        try:
            ...
            assert self.stimulus_dataset_repository is not None
            assert self.acquisition_session_repository is not None
            assert self.analysis_result_repository is not None
    ...
    async def start(self):
        ...
        try:
            ...
            await self.state_persistence.start_auto_save()
    ...
    async def shutdown(self):
        ...
        try:
            ...
            if self.state_persistence:
                await self.state_persistence.stop_auto_save()
