...
class IPCServer:
    ...
    async def start(self) -> None:
        ...
        # Check if stdin is available and readable before attempting connection
        stdin_available = False
        try:
            # Check if stdin is a valid file descriptor and is readable
            import os
            import stat
            stdin_fd = sys.stdin.fileno()
            stdin_stat = os.fstat(stdin_fd)
            stdin_available = stat.S_ISFIFO(stdin_stat.st_mode) or stat.S_ISREG(stdin_stat.st_mode)
        ...
        if stdin_available:
            ...
            except (OSError, asyncio.InvalidStateError) as e:
                # stdin connection failed - fall back to broadcast mode
                logger.info(f"IPC server stdin connection failed, using broadcast mode: {e}")
                asyncio.create_task(self._process_outgoing_messages())
                logger.info("IPC server started - broadcasting mode only")
        else:
            # stdin not available - start in broadcast mode only
            logger.info("IPC server starting in non-interactive mode (stdin not available)")
            asyncio.create_task(self._process_outgoing_messages())
            logger.info("IPC server started - broadcasting mode only")
