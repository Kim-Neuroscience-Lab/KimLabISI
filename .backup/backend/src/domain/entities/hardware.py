...
class HardwareComponent:
    ...
    def __init__(
        ...
        logger.debug("Created hardware component: %s (%s)", self.hardware_id, self.name)
    ...
    def status(self, new_status: HardwareStatus):
        ...
        if new_status != self._status:
            ...
            self._last_status_update = datetime.now()
            logger.info("Hardware %s status changed to %s", self.hardware_id, new_status.value)
    ...
    def capabilities(self, capabilities: HardwareCapabilities):
        ...
        logger.debug("Updated capabilities for %s", self.hardware_id)
    ...
    def configuration(self, config: HardwareConfiguration):
        ...
        logger.debug("Updated configuration for %s", self.hardware_id)
...
class Camera(HardwareComponent):
    ...
    def enable_cooling(self, target_temperature: float = -10.0):
        ...
        logger.info("Enabled cooling for camera %s to %sC", self.hardware_id, target_temperature)

    def disable_cooling(self):
        """Disable camera cooling"""
        self._cooling_enabled = False
        self._configuration.temperature_setpoint = None
        logger.info("Disabled cooling for camera %s", self.hardware_id)

    def set_exposure(self, exposure_ms: float):
        """Set camera exposure time"""
        self._configuration.exposure_time_ms = exposure_ms
        logger.debug("Set exposure to %sms for camera %s", exposure_ms, self.hardware_id)

    def set_binning(self, x_bin: int, y_bin: int):
        """Set camera pixel binning"""
        self._configuration.binning = (x_bin, y_bin)
        logger.debug("Set binning to %sx%s for camera %s", x_bin, y_bin, self.hardware_id)
...
class HardwareSystem:
    ...
    def __init__(self, system_id: str = "isi_macroscope_system"):
        ...
        logger.info("Initialized hardware system: %s", system_id)
    ...
    def add_component(self, component: HardwareComponent):
        ...
        logger.info("Added component %s to system %s", component.hardware_id, self.system_id)
    ...
    def remove_component(self, hardware_id: str) -> bool:
        ...
        if hardware_id in self._components:
            ...
            logger.info("Removed component %s from system %s", hardware_id, self.system_id)
            return True
        return False
    ...
    def calculate_system_health(self) -> float:
        ...
        if not self._components:
            # In development mode or when no hardware is connected,
            # consider the system healthy (no hardware = no hardware problems)
            return 1.0
    ...
    def save_to_file(self, file_path: Path):
        ...
        logger.info("Saved hardware system state to %s", file_path)
    ...
    def load_from_file(cls, file_path: Path) -> HardwareSystem:
        ...
        logger.info("Loaded hardware system from %s", file_path)
        return system
