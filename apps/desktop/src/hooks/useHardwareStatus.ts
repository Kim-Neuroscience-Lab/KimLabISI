...
export const useHardwareStatus = ({
  ...
  const testAllSystems = useCallback(async (force: boolean = false) => {
    ...
    try {
      ...
      if (response?.success && response?.hardware_status) {
        ...
        hookLogger.error('Health check failed:', response?.error || 'Unknown error')
      ...
      hookLogger.error('Centralized health check failed:', error)
      updateHardwareStatus({
        camera: 'error',
        display: 'error',
        stimulus: 'error',
  ...
  useEffect(() => {
    if (isConnected && isReady && sendCommand) {
      ...
      testedSystems.current.clear()
      // Queue health check using microtask to avoid blocking render
      queueMicrotask(() => testAllSystems(true))
    }
  ...
  const selectCamera = useCallback(async (cameraName: string) => {
    ...
    try {
      ...
      hookLogger.error('Failed to select camera:', error)
    }
  }, [sendCommand])
  ...
  const selectDisplay = useCallback(async (displayId: string) => {
    ...
    try {
      ...
      hookLogger.error('Failed to select display:', error)
    }
  }, [sendCommand])
