...
import { ParameterConfig } from '../config/parameterConfigs'
...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  const handleCameraChange = React.useCallback(async (newCameraParams: Record<string, any>) => {
    ...
    if (newCameraParams.selected_camera && newCameraParams.selected_camera !== cameraParams.selected_camera) {
      try {
        ...
        // Update camera parameters with detected values (only if detected)
        const updatedParams = {
          ...cameraParams,
          ...newCameraParams
        }

        // Only set detected values if they exist and are valid
        ...
        // Only mark as auto-detected if we actually detected ALL values successfully
        const hasAllDetectedValues = capabilities.frameRate !== undefined && capabilities.frameRate > 0 &&
                                   capabilities.width !== undefined && capabilities.width > 0 &&
                                   capabilities.height !== undefined && capabilities.height > 0
        setCameraParamsAutoDetected(hasAllDetectedValues)
      } catch (error) {
      ...
      // Just update the parameters normally
      setCameraParams(prev => ({ ...prev, ...newCameraParams }) as CameraParameters)
    }
  }, [cameraParams.selected_camera])

  // Fetch cameras when component mounts or connection status changes
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    if (availableCameras.length > 0 && !cameraParamsAutoDetected && cameraParams.selected_camera === '') {
      const firstCamera = availableCameras[0]
      handleCameraChange({ selected_camera: firstCamera })
    }
  }, [availableCameras, cameraParamsAutoDetected, cameraParams.selected_camera, handleCameraChange])
