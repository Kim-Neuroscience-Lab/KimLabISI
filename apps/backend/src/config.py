...
class AppConfig:
    ...
    def from_file(path: str) -> "AppConfig":
        ...
        return AppConfig(
            ...
            analysis=AnalysisConfig(
                magnitude_threshold=current.get("analysis", {}).get("magnitude_threshold", 0.1),
                phase_filter_sigma=current.get("analysis", {}).get("phase_filter_sigma", 0.5),
                smoothing_sigma=current.get("analysis", {}).get("smoothing_sigma", 1.0),
                gradient_window_size=current.get("analysis", {}).get("gradient_window_size", 3),
                response_threshold_percent=current.get("analysis", {}).get("response_threshold_percent", 5.0),
                vfs_threshold_sd=current.get("analysis", {}).get("vfs_threshold_sd", 2.0),
                ring_size_mm=current.get("analysis", {}).get("ring_size_mm", 1.0),
