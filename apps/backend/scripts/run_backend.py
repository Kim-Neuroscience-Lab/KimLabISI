...
def find_backend_root():
    """Locate the backend app directory containing pyproject.toml"""
    current = Path(__file__).resolve()
    for ancestor in [current] + list(current.parents):
        candidate = ancestor / "pyproject.toml"
        if candidate.exists() and (ancestor / "src" / "isi_control").exists():
            return ancestor
    raise RuntimeError("Could not locate backend pyproject.toml")
